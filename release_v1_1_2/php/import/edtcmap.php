...
// Retrieve the record for this cmap
if ($cmap_id) {
    $cmap = getRow('cor_tbl_cmap', $cmap_id, FALSE);
    //printPre($cmap);
} else {
    $cmap = FALSE;
}
...
// put in a prefilled form
if ($cmap_id && $cmap) {
    // form
    $form = "<div id=\"editcmap_home\">";
    ...
    $form .= "<fieldset>";
    $form .= "<input type=\"hidden\" name=\"update_db\" value=\"edtcmp\" />";
    // Contain the input elements in a list
    $form .= "<ul>\n";
    $form .= "<li class=\"row\">";
    $form .= "<label class=\"form_label\">Nickname</label>";
    $form .= "<input type=\"text\" name=\"nickname\" value=\"{$cmap['nname']}\" />\n";
    $form .= "</li>\n";
    $form .= "<li class=\"row\">";
    $form .= "<label class=\"form_label\">Description</label>";
    $form .= "<textarea id=\"cmap_desc\" name=\"cmap_desc\" rows=\"4\" cols=\"30\" >{$cmap['description']}";
    $form .= "</textarea>\n";
    $form .= "</li>\n";
    $form .= "<li class=\"row\">";
    $form .= "<label class=\"form_label\">Source DB</label>";
    $form .= "<select name=\"source_db\">\n";
    $form .= "<option value=\"{$cmap['sourcedb']}\">{$cmap['sourcedb']}</option>\n";
    while ($row = $db_list->fetch(PDO::FETCH_ASSOC)) {
        if ($row['Database'] != $cmap['sourcedb']) {
    ...
    $form .= ddSimple($cmap['stecd'], $cmap['stecd'], 'cor_tbl_ste', 'id', 'ste_cd', '', 'code');
    $form .= "</li>\n";
    $form .= "<li class=\"row\">";
