...
var conf_loading = 'flowloading';        // Default is 'loading'
...
function highlightImageflow (element){
    
    image = document.getElementById('webthumb_' + element);
    selection = document.getElementById("file_selection");
    text = selection.innerHTML;
    text_split = explode(" ",text);
    
    if (image.style.border == "5px solid red"){
        for(var i = 0, l = text_split.length; i < l; i++) {
           if(text_split[i] == element) {
                delete text_split[i];
            }
        }
        ...
        text_split.push(element);
    }
     form_val = str_replace(","," ", implode(',',text_split));
     form_val = ltrim(form_val);
     form_val = rtrim(form_val);
     selection.innerHTML = form_val;
}
...
{    
    ...
    lightbox_url = 'data/files/' + img_div.childNodes.item(array_images[caption_id]).childNodes.item(0).getAttribute('id') + '.jpg';
    caption = img_div.childNodes.item(array_images[caption_id]).childNodes.item(0).getAttribute('alt');
    if (caption == '') caption = '&nbsp;';
    caption_div.innerHTML = caption + '\n<a href="' + lightbox_url + '" rel="lightbox[]" title="' + caption + '">zoom</a>';
...
function moveTo(x)
{
    ...
    {
        var image = img_div.childNodes.item(array_images[index]).childNodes.item(0);
...
function refresh(onload)
{
    ...
    for (var index = 0; index < max; index++)
    { 
        var image = img_div.childNodes.item(index).childNodes.item(0);
        if (image.nodeType == 1)
        {
